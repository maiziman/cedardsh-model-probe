...
`@maiziman/dsh-model-capabilities` is an official-format DeepSeek Harness Bundle installed beside the upstream packages. It observes the public `llm-pi-ai` settings and Credentials services without importing or replacing the official adapter. Provider metadata is the preferred evidence; explicit model and provider declarations always win. Bounded active probes are allowed only for exact `localhost` and literal loopback, private-network, link-local, or unique-local addresses by default. Requests reject redirects. Reasoning probes bracket candidate-effort requests with the same invalid-effort negative control. A 400/422 rejection or a successful response without reasoning is a valid negative control; reasoning returned for that invalid value makes the route ambiguous. Every saved effort independently returns reasoning evidence. Image input is saved only when the model returns the exact color word for both randomly ordered embedded private PNG challenges; explanatory or ambiguous replies fail.
...
The Bundle's seven npm package files ship from the CedarDSH Model Probe repository in `CedarDSH-Model-Probe-v<version>.tgz`. Releases also contain a Windows ZIP with that TGZ, one-click install and removal files, and a bilingual quick start. Both one-click files require the user to confirm that Desktop and command-line Web sessions are closed. The installer copies the versioned TGZ into the Web profile's `plugin-packages` directory and registers a profile-relative file dependency through the official `dsh plugin` command, so moving the complete portable folder preserves the dependency. The removal file invokes the same official command, deletes only plugin-owned package archives, and preserves model settings. CedarDSH Desktop contains and auto-installs no plugin. The plugin remains outside every upstream package, so the upstream dsh source and adapter stay unchanged.
...
- Registering a TGZ directly from the executable directory writes an absolute dependency path that can break when the portable folder moves.
- Automatically ending CedarDSH before profile changes could discard session state, so the one-click files wait for the user to close it.
...
A sparse local endpoint that never returns positive evidence can be probed again after a later application restart. Requests remain background, bounded, configurable, and restricted to local endpoints by default. Version 0.1.3 installs and runs against exact upstream tag `dsh-v0.1.2-alpha.2` (`0a53fb55`). Before publication, a Windows runner executes both one-click files with the published dsh package in a path containing spaces, covering installation, repeat installation, removal, repeat removal, manifest registration, installed files, and stale package cleanup. The repository also tests package contents, negative-control behavior, one-click file invariants, and clean tarball installation. Later pre-release changes to the settings service or `llm-pi-ai` schema may require another plugin update.
